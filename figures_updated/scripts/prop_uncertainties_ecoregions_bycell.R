...
data_barplot <- foreach(sp = species_list, .combine=rbind) %do% {
  ...
  anova_ecoregions <- sapply(ecoregions, function(ecoreg){
    ...
    c(ecoreg, t(global(ecoreg_anova, mean, na.rm = TRUE)))
  }) %>% t() %>% data.frame() 
  names(anova_ecoregions) <- c("ecoregion", cols)
  
  data_plot <- anova_ecoregions %>%
    # tidyr::pivot_longer(cols = cols, names_to = "source", values_to = "prop") %>%
    left_join(saved_data_species %>% filter(species == sp, year == 2090), by = c("ecoregion")) %>%
    mutate(ecoregion = factor(ecoregion, levels = rev(c("Boreal", "Alpine", "Continental", "Atlantic", "Mediterranean")))) %>%
    mutate_at(cols, as.numeric) %>%
    mutate(tot = ssp + gcm + gcm.ssp + sdm + sdm.climate + residuals, species = sp)
