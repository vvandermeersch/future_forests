...
anovaprop_r <- rast(lapply(cols, function(i)
  rast(anova_ss[c("x", "y", i)], type = "xyz")))/anovatot

crs(anovaprop_r) <- "EPSG:4326"
anovaprop_rp <- anovaprop_r %>% project("EPSG:3035") 

maps_prop_uncertainties <- ggplot() +
  facet_wrap(~lyr, 
             labeller = labeller(lyr = c(`typemax1.num` = "First major source", `typemax2.num` = "Second major source"))) +
  geom_spatraster(data = subset(anovaprop_rp, 1:3)) +
...
prop_countries <- sapply(countries, function(country){
  cat(country)
  ...
  country_anova <- crop(anovaprop_r, country_map, mask = TRUE)
...
names(prop_countries) <- c("country", cols, "mean", "sd", "median", "boot.ci.min", "boot.ci.max")

data_plot <- prop_countries %>%
...
barplot_prop_uncertainties <- ggplot(data = data_plot) +
  ...
  geom_rect(aes(ymin = mean - 1.645*sd * (climate + sdm.climate + sdm + species + species.all)/tot, 
                ...
                xmin = as.numeric(country)-0.3, xmax = as.numeric(country)+0.3, fill = "species.all"),
            color = NA) +
  geom_rect(aes(ymin = mean - 1.645*sd * (climate + sdm.climate + sdm + species)/tot, 
                ...
                xmin = as.numeric(country)-0.3, xmax = as.numeric(country)+0.3, fill = "species"),
            color = NA) +
  ...
  geom_rect(aes(ymin = mean - 1.645*sd * (climate)/tot, 
                ...
                xmin = as.numeric(country)-0.3, xmax = as.numeric(country)+0.3, fill = "climate"),
            color = NA) +
