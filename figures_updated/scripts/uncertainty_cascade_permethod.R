...
unc_cascade <- ggplot() +
  ...
  geom_line(data = data %>% dplyr::filter(y < 2 & method == "inverse") %>% unique(),
            aes(x = x*100, y = y, group = paste0(cal, method, ssp, gcm), 
  ...
  geom_line(data = data %>% dplyr::filter(y > 0 & y < 3),
            aes(x = x*100, y = y, group = paste0(method, ssp, gcm), color = ssp), alpha = 0.7, linewidth = 0.6) +
  ...
  geom_text(data = data %>% filter(y > 2) %>% mutate(y = 3.3) %>% select(year, ssp, x, y) %>% unique() %>% filter(year == "2080-2100"),
            aes(x = x*100, y = y, label = ifelse(ssp == "ssp245", "SSP2", "SSP5"), color = ssp), size = 2.3) +
  geom_point(data = data %>% dplyr::filter(y == 2) %>% dplyr::select(-c(gcm, ref, cal)) %>% unique(),
             aes(x = x*100, y = y, shape = method), size = 2, fill = "white", color = "white") +
  geom_point(data = data %>% dplyr::filter(y == 2) %>% dplyr::select(-c(gcm, ref, cal)) %>% unique(),
             aes(x = x*100, y = y, shape = method, color = ssp), size = 2, fill = NA) +
