...
simulations$method <- factor(ifelse(simulations$cal == "expert", "expert",
                                    ifelse(simulations$cal %in% csdms, "csdm", 
                                           ifelse(grepl("partial", simulations$cal, fixed = TRUE),
                                                  "partial",
                                                  "inverse"))),
                             levels = c("expert", "partial", "inverse", "csdm"))
...
cascade1 <- ggplot() +
  ...
  theme_bw() + 
  geom_text(data = data.frame(year = "2020-2040", x = c(0.33,1.33),
                              text = c("Individual\ncalibration", "GCMs multi-\ncalibration means")),
            aes(x = x, label = text, y = ytext), color = "grey50", size = 3.25, angle = 90) +
  #scale_color_manual(values = c(ssp245 = "#f79320", ssp585 = "#951b1e"), guide = "none") +
  scale_color_manual(values = c(expert = "#018530", partial = "#d2c024",inverse = "#995D81", csdm = "#457b9d"),
                     breaks = c("csdm", "inverse", "expert"),
...
cascade2 <- ggplot() +
  ...
  # scale_color_manual(values = c(ssp245 = "#f79320", ssp585 = "#951b1e"), guide = "none") +
  scale_color_manual(values = c(expert = "#018530", partial = "#d2c024", inverse = "#995D81", csdm = "#457b9d"),
                     breaks = c("csdm", "inverse", "partial", "expert"),
                     labels = c("Correlative SDM", "Fitted PEM", "Partial. fit. PEM", "Expert PEM")) + 
  theme(axis.line.x = element_blank(), axis.text.x = element_blank(), axis.title.x = element_blank(),
        panel.grid = element_blank(), axis.ticks.x = element_blank(), panel.border = element_blank(),
        axis.line.y = element_line(color = "grey20"),
